...
app.controller('IdentityCtlEx', ['$scope', '$stateParams', 'IdentityService',
  'ngDialog', 'FlowServiceEx', 'DegradeServiceEx', 'AuthorityRuleServiceEx', 'ParamFlowServiceEx', 'MachineService',
    ...
    function saveFlowRule() {
      ...
      FlowService.newRule(flowRuleDialogScope.currentRule).success(function (data) {
        if (data.code === 0) {
          ...
          let url = '/dashboard/flow/ex/' + $scope.app;
          $location.path(url);
        } else {
          alert('失败：' + data.msg);
        }
    ...
    function saveDegradeRule() {
      ...
      DegradeService.newRule(degradeRuleDialogScope.currentRule).success(function (data) {
        if (data.code === 0) {
          ...
          var url = '/dashboard/degrade/ex/' + $scope.app;
          $location.path(url);
        } else {
          alert('失败：' + data.msg);
        }
      ...
      function saveAuthorityRule() {
          ...
          AuthorityRuleService.addNewRule(ruleEntity).success((data) => {
              if (data.success) {
                  ...
                  let url = '/dashboard/authority/ex/' + $scope.app;
                  $location.path(url);
              } else {
      ...
      function saveParamFlowRule() {
          ...
          ParamFlowService.addNewRule(ruleEntity).success((data) => {
              if (data.success) {
                  ...
                  let url = '/dashboard/paramFlow/ex/' + $scope.app;
                  $location.path(url);
              } else {
