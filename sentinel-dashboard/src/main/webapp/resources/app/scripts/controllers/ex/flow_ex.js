...
app.controller('FlowControllerEx', ['$scope', '$stateParams', 'FlowServiceEx', 'ngDialog',
  'MachineService',
  function ($scope, $stateParams, FlowServiceEx, ngDialog,
    MachineService) {
    ...
    function getMachineRules() {
      ...
      FlowServiceEx.queryMachineRules($scope.app, mac[0], mac[1]).success(
        function (data) {
    ...
    $scope.saveRule = function () {
      if (!FlowServiceEx.checkRuleValid($scope.currentRule)) {
        return;
      }
    ...
    function deleteRule(rule) {
      FlowServiceEx.deleteRule(rule).success(function (data) {
        if (data.code == 0) {
          getMachineRules();
          confirmDialog.close();
        } else {
    ...
    function addNewRule(rule) {
      FlowServiceEx.newRule(rule).success(function (data) {
        if (data.code === 0) {
          getMachineRules();
    ...
    function saveRule(rule, edit) {
      FlowServiceEx.saveRule(rule).success(function (data) {
        if (data.code === 0) {
          getMachineRules();
          if (edit) {
            flowRuleDialog.close();
